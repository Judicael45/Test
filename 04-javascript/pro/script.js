...
/*
 ...
 soit undefined s elle ne retourne rien.
 Il s'avêre que fetch retourne bien quelque chose, une promesse.
*/
...
.cath() qui prend aussi une fonction callback mais qui sera appelé si la promesse est rejeté (échec).
.finally() qui on l'aura compris,prend une fonction callback,
qui sera appelé une fois la promesse traitée (échec ou réussite).
*/
...
/*
   Il est possible de résodre plusieurs promesses en même temps.
   Pour cela on fera appel à la méthode all() de l'objet "Promise"
   à laquelle on donnera un tableau de toute les promesse que l'on souhaite résoudre.
...
Promise.all([r1, r2]).then(res => {
    ...
    res.forEach(r => {
        // Je traitre chaque réponse comme je le ferais sur un fetch classique.
        if (r.ok) {
...
/*
  On trouvera aussi les méthodes "Promise.race()" et "Promise.any()"
  qui prendront elles aussi un tableau de promesse.
  Mais à la différence de ".all()" elles ne rendront que la plus rapide à s'éxécuter.
  La différence entre "race" et "any" se fait au niveau du ".catch()" :
  - race lancera le catch si la plus rapide des promesses échoue.
  - any passera à la promesse suivante tant que la précédente échoue et ne lancera le tach que si elles échouent toute.
  */
/*
...
function viande1() {
    setTimeout(()=>console.log("le pain est grillé et placé"),3000)
}
function salade1() {
