...
/*
  ...
  Mais il est possible de fare de la "3d" avec "webgl" par exemple.
*/
...
/*
   fillRect et strokeRect permettent de dessiner un rectangle avec les paramètres suivants :
...
/*
  ...
  Elles prennnent un string contenant une couleur.
  (rbg,hexadecimal,mot clef)

  Le changement ne s'applique qu'aux dessins qui suivent et non aux précédents.
*/
ctx.fillStyle = "rbg(156,78,94, 0.5)"
ctx.fillStyle = "red"
ctx.fillRect(25,5,78,95)
...
ctx.lineWidth = 8
ctx.stroke()
// fill permet de remplir la forme précédement déssiné.
ctx.fill()
...
ctx.beginPath()
    /*
     ...
     position x,position y, taille du rayon, position de départ du radiant (0 pour un cercle complet)
     position de fin du radiant (Math.PI*2 pour un cercle complet)
    */

ctx.arc(89,90,42,0,2*Math.PI)
ctx.stroke()
// clearRect permet d'effacer ce qui se trouve das la zone rectangulaire selectionner.
...
/*
 ...
 faire une function qui dessine un cercle rayon et de position défini par les variable précédentes.


 Ajouter des conditions : 
 si x + le rayon est plus grand que la largeur du canvas , ou si x - le rayonn plus petit que 0.
...
img.onload = ()=>ctx.drawImage(img,400,410,100,100)
