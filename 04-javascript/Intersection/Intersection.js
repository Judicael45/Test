...
const option = {
    ...
    */
    ...
    Avec un nombre positif, la détéction sera déclenché hors de l'écran.
    Avec un nombre négatif, la détection sera déclenché dans une zone rédute de l'écran.
    ...
    /*
     ...
     !! Attention d'avoir un chiffre possible, si votre élément es plus grand que votre viewport, il n'attendra jamais 1.
     */
    //treshold: 0.15 
}
/*
Inersection Observer est un objet permettant de détecter lorsqu'un élément html rentre dans le viewport lors
du scrolling.
Il prend obligatoirement une fonction callback en premier argument suivi 
d'optionnellement un objet contenant ses option.
*/
...
{
    /*
    La fonction donné en callback à l'observer sera lancé à chaque fois
     que l'objet observé rentre ou sort du viewport.
    */
    let entry = entries[0]
    console.log(entry);
    /*
    Dans une entrée on trouvera les informations suivantes : 
    target => la cible qui a été détécté dans le viewport
    isItersecting => boolean qui indique si l'élément est dans le viewport
...
function indicatorAnimation(){
    ...
    //    console.log(prc);
       indicator.style.transform= `scaleX(${prc})`
        ...
        indicator.getElementsByClassName.transform = "scaleX(0)"
    }
}
/*
