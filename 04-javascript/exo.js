...
/*
Par défaut, une fonction est déclaré avec le mot clef "fonction"
Il es suivi du nom de la fonction.
puis de parenthèse qui acceuillerons les possibles arguments.
Et enfin les accolades qui contiendrons le corps de la fonction.

Une fonction déclaré ainsi, peut être appelé avant ou après
sa déclaration.
...
/*
    ...
    comme en placant une fonction dans une variable.
 */
...
bonsoir("Maurice")
//Si des paramètrs en trop sont donné, ils seront juste ignoré.
...
function goodMorning(...noms) {
   ...
   // *join fait de même mais en séparant les élément du tableau par le string donné en paramètre.
...
Lorsque l'on donne une fonction callback,
on donne seulement son nom et on ne met pas les parenthèses, ce n'est pas un appel de fonction.
*/
...
// La fonction fléché à une seule instruction, (sans accolade)
//fait ce qu'on appelle un "retour implicite", c'est à dire que même si le return n'est pas écrit, l'instruction est bien retourné.
...
dr.forEach((princesse)=>{
   if (princesse<=0)return {
